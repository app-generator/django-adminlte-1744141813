# -*- encoding: utf-8 -*-
"""
Copyright (c) 2019 - present AppSeed.us
"""

from django.db import models
from django.utils import timezone
from django.contrib.auth.models import User
from django.utils.translation import gettext_lazy as _

# Create your models here.

class UserProfile(models.Model):

    user = models.OneToOneField(User, on_delete=models.CASCADE)

    #__PROFILE_FIELDS__
    bio = models.TextField(max_length=255, null=True, blank=True)
    foto = models.IntegerField(null=True, blank=True)

    #__PROFILE_FIELDS__END

    def __str__(self):
        return self.user.username
    
    class Meta:
        verbose_name        = _("UserProfile")
        verbose_name_plural = _("UserProfile")

#__MODELS__
class Produto(models.Model):

    #__Produto_FIELDS__
    nome = models.CharField(max_length=255, null=True, blank=True)
    tipo = models.ForeignKey(TipoProduto, on_delete=models.CASCADE)
    valor = models.IntegerField(null=True, blank=True)
    tamanho = models.IntegerField(null=True, blank=True)

    #__Produto_FIELDS__END

    class Meta:
        verbose_name        = _("Produto")
        verbose_name_plural = _("Produto")


class Tipoproduto(models.Model):

    #__Tipoproduto_FIELDS__
    nome = models.CharField(max_length=255, null=True, blank=True)

    #__Tipoproduto_FIELDS__END

    class Meta:
        verbose_name        = _("Tipoproduto")
        verbose_name_plural = _("Tipoproduto")



#__MODELS__END
